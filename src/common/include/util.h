...
// 用于资源管理的类
// 在作用域结束时执行给定的函数，这在资源清理时非常有用（如文件关闭、锁释放）
template <class F>
class DeferClass {
 public:
...
// 创建一个DerferClass实例  在当前作用域结束时执行，有助于简化资源管理代码
#define _MAKE_DEFER_(line) DeferClass _CONCAT(defer_placeholder, line) = [&]()  

#undef DEFER
#define DEFER _MAKE_DEFER_(__LINE__) 

void DPrintf(const char* format, ...);  // 调试打印

void myAssert(bool condition, std::string message = "Assertion failed!");

// 字符串格式化函数  类似printf，但返回一个格式化后的字符串
...
// 返回当前高精度时间点，用于raft算法的定时器和超时机制
std::chrono::_V2::system_clock::time_point now();

// 生成一个随机的选举超时时间，用于领导者选举
std::chrono::milliseconds getRandomizedElectionTimeout();
// 使当前线程暂停指定的毫秒数，用于等待或延迟操作。
void sleepNMilliseconds(int N);

// ////////////////////////异步写日志的日志队列
// read is blocking!!! LIKE  go chan
// 这是一个线程安全的队列类，用于在多个线程之间传递数据。在Raft算法中，用于日志条目的传递
template <typename T>
class LockQueue {
 public:
...
// 操作类，表示键值存储中的操作（如获取、放置、追加）；还支持序列化和反序列化
class Op {
 public:
